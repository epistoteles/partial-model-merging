...
def plot_correlation_histogram(model_name_a: str, model_name_b: str = None):
    ...
    fig.suptitle(
        ...
        f"{dataset_a}, {model_type_a}{size_a}, {'batch norm, ' if bn_a else ''}{width_a}×width",
        y=1.05,
    )
    ...
    for i in range(len(chosen_corrs)):
        ...
        if last_xtick / xlim > 0.9:
            last_xtick = find_nice_number(xlim * 0.9)  # otherwise the last xtick will overlap with the next one
